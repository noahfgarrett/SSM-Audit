...
import { extoRev21Canonical, extoRev21EffectiveDiscipline, extoRev21Norm, extoRev21UpnCandidates } from '../exto/rev21-contract.js'
...
export const SSM_AUDIT_RULES=Object.freeze({
  ...
  crossDiscipline:auditRule('parent.cross-discipline','sop','structure','Parent and child share a discipline','Cross-discipline structural relationships need review unless they are approved controls or instrumentation children.'),
...
export function runSsmAudit(snapshot){
  ...
  for(const row of rows){
    ...
    if(extoRev21EffectiveDiscipline(row.discipline)==='FACILITIES MONITORING SYSTEM'&&extoRev21Norm(row.discipline)!=='FACILITIES MONITORING SYSTEM'){checks++;const candidates=extoRev21UpnCandidates(row.equipmentId);add(SSM_AUDIT_RULES.icDiscipline,'blocker',row,{field:'Discipline',why:'I&C does not match the allowed Cx discipline mapping.',actual:`${row.discipline}; tag UPN candidates: ${candidates.join(', ')||'none'}`,expected:'FACILITIES MONITORING SYSTEM with one tag-derived UPN',recommendation:candidates.length===1?`Use UPN ${candidates[0]} and its allowed System Name.`:'Resolve the tag-derived UPN before assigning Discipline and System Name.'});}
    if(row.systemName&&row.upn){checks++;const match=extoRev21Norm(row.systemName).match(/^([0-9]+)(?:\s|$)/),systemUpn=match&&match[1],assigned=extoRev21Norm(row.upn);if(!systemUpn||extoRev21Norm(systemUpn)!==assigned)add(SSM_AUDIT_RULES.systemUpn,'blocker',row,{field:'System Name',why:systemUpn?'System Name identifies a different UPN than the row assignment.':'System Name does not identify its assigned UPN.',actual:`UPN ${row.upn}; System ${row.systemName}`,expected:`System Name beginning with UPN ${row.upn}`,recommendation:'Correct the UPN or select the System Name assigned to that UPN.'});}
    ...
    checks++;if(!parentId)add(SSM_AUDIT_RULES.blankParent,'blocker',row,{field:'Closest Parent',why:'This equipment has no structural parent or System Name root.',expected:'Equipment parent or the row System Name',recommendation:'Assign the correct same-UPN parent; use System Name for a root.'});
      ...
      if(parent){
        ...
        const approvedControlsChild=extoRev21EffectiveDiscipline(row.discipline)==='FACILITIES MONITORING SYSTEM'||auditIsControlEquipment(row);
        if(childDiscipline&&parentDiscipline&&childDiscipline!==parentDiscipline&&!approvedControlsChild)add(SSM_AUDIT_RULES.crossDiscipline,'warning',row,{field:'Closest Parent',why:'This structural parent crosses Discipline partitions.',actual:`${row.discipline} → ${parent.discipline}`,expected:`Parent in ${row.discipline}, an approved controls child, or a documented exception`,recommendation:'Confirm the exception or represent the crossing as a dependency.',relatedEquipmentId:parent.equipmentId});
