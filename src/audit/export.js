...
export function exportSsmAuditXlsx(){
  ...
  const findingHeaders=['Severity','Category','Rule Source','Rule','Rule ID','Equipment ID','Sheet','Row','Field','Why flagged','Found','Expected','Recommended correction','Fingerprint'];
  const findingRows=result.findings.map(finding=>[
    finding.severity.toUpperCase(),finding.category,finding.rule.source,finding.rule.title,finding.rule.id,finding.equipmentId,finding.sheet,finding.row,finding.field,finding.why,
  ...
  findingsSheet['!cols']=[{wch:11},{wch:18},{wch:20},{wch:32},{wch:34},{wch:28},{wch:22},{wch:9},{wch:25},{wch:58},{wch:45},{wch:45},{wch:58},{wch:12}];
  styleHeaderRow(findingsSheet);findingsSheet['!autofilter']={ref:`A1:N${Math.max(1,findingRows.length+1)}`};addSheet(workbook,findingsSheet,'Findings');
  ...
  const ruleRows=[['Rule Source','Rule','Rule Statement','Rule ID','Category','Blockers','Errors','Warnings','Advisories','Total'],
    ...[...byRule.values()].sort((a,b)=>b.total-a.total||a.rule.id.localeCompare(b.rule.id)).map(entry=>[entry.rule.source,entry.rule.title,entry.rule.statement,entry.rule.id,entry.rule.category,entry.blocker,entry.error,entry.warning,entry.info,entry.total])];
  const rulesSheet=XLSX.utils.aoa_to_sheet(ruleRows);rulesSheet['!cols']=[{wch:20},{wch:32},{wch:62},{wch:34},{wch:20},{wch:11},{wch:11},{wch:11},{wch:11},{wch:11}];
