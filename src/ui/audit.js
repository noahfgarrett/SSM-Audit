...
function hierarchyFilterSet(hierarchy){
  ...
  for(const candidate of hierarchy.groups.systems){if(building!=='all'&&candidate.buildingKey!==building||discipline!=='all'&&candidate.disciplineKey!==discipline||system!=='all'&&candidate.key!==system)continue;addAncestors(candidate);addDescendants(candidate);}
  return visible;
}
function hierarchySearchSets(hierarchy,query){
  if(!query)return {visible:null,direct:null};const visible=new Set(),direct=new Set(),addAncestors=node=>{const seen=new Set();while(node&&!seen.has(node.key)){seen.add(node.key);visible.add(node.key);node=hierarchy.nodeByKey.get(node.parentKey);}},addDescendants=node=>{const stack=[node],seen=new Set();while(stack.length){const current=stack.pop();if(!current||seen.has(current.key))continue;seen.add(current.key);visible.add(current.key);for(const child of current.children)stack.push(hierarchy.nodeByKey.get(child));}};
  for(const node of hierarchy.nodeByKey.values())if(node.searchKey.includes(query)){direct.add(node.key);addAncestors(node);if(node.children.length)addDescendants(node);}
...
function wireHierarchyRows(){
  $$('[data-hierarchy-toggle]').forEach(button=>button.onclick=event=>{event.stopPropagation();const expanded=new Set(S.session.hierarchyExpandedKeys||[]),key=button.dataset.hierarchyToggle;if(expanded.has(key))expanded.delete(key);else expanded.add(key);setHierarchyExpanded(expanded);renderHierarchyRows();});
...
function hierarchyOptions(items,selected,label){return `<option value="all">${esc(label)}</option>`+items.map(item=>`<option value="${esc(item.key)}" ${selected===item.key?'selected':''}>${esc(item.type==='system'?`${item.upn} · ${item.label}`:item.label)}</option>`).join('');}
